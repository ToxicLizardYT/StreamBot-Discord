...
urls = [  # List of gif urls
    ...
    "https://media.giphy.com/media/hiFfdSwE5oF7q/giphy.gif",
    "https://media.giphy.com/media/xlGYf1RUbYYes/giphy.gif",
    "https://media.giphy.com/media/SL9eV3oQYJUtj97WlA/giphy.gif"
]
...
async def background_loop():  # Gif of the hour
    while True:
...
@client.event
async def on_ready():  # When bot is up
...
@client.event
async def on_message(message):  # When message is sent
    Time = datetime.datetime.now(pytz.timezone("US/Central"))  # Gets time when the message was sent
    if message.content.upper().startswith("$TIME"):  # Gets time until 7pm CDT
    ...
    elif message.content.upper().startswith("$GIF"):  # Sends random gif from the urls list
    ...
    elif message.content.upper().startswith("$INFO"):  # Gives user info
    ...
    elif message.content.upper().startswith("$HELP"):  # Basic help command
        args = message.content.split(" ")  # Makes parameter list
